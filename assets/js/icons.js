/**
 * 단색 라인 아이콘 모음.
 * 모든 아이콘은 stroke="currentColor" 를 쓰므로 놓인 자리의 글자색을 그대로 따른다.
 * (남색 사이드바에서는 흰색, 흰 배경 탭바에서는 회색/파랑으로 보인다)
 */

const PATHS = {
    // 주문정보등록 - 클립보드
    orders: `
        <path d="M9 4h6v2H9z" />
        <path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2" />
        <path d="M8.5 11.5h7M8.5 15.5h4.5" />`,

    // 주문처리현황 - 막대 그래프
    status: `
        <path d="M3.5 20.5h17" />
        <path d="M7 20.5v-5.5M12 20.5V6M17 20.5v-9" />`,

    // 출고주문처리 - 상자와 체크
    shipping: `
        <path d="M3.2 7.6 12 3l8.8 4.6v8.8L12 21l-8.8-4.6z" />
        <path d="M3.2 7.6 12 12.2l8.8-4.6M12 12.2V21" />
        <path d="M8.6 14.4l2 2 4-4" />`,

    // 상차리스트 - 트럭
    loading: `
        <path d="M2.5 7.5a1 1 0 0 1 1-1h9a1 1 0 0 1 1 1v9h-11z" />
        <path d="M13.5 10.5h3.6a1 1 0 0 1 .8.4l2.6 3.3v2.3h-7z" />
        <circle cx="7" cy="18.5" r="2" />
        <circle cx="17" cy="18.5" r="2" />`,

    // 유통가공작업 - 상자 위에 라벨 한 장을 붙인 모양 (orders 의 클립보드·stow 의 파렛트와 구분)
    processing: `
        <path d="M3.5 9.5h17v10a1 1 0 0 1-1 1h-15a1 1 0 0 1-1-1z" />
        <path d="M3.5 9.5 6 4.5h12l2.5 5" />
        <path d="M9.5 13.5h5v4.5h-5z" />
        <path d="M12 9.5v4" />`,

    // 이슈등록 - 경고 삼각형
    issues: `
        <path d="M10.3 4.4 2.6 18a2 2 0 0 0 1.7 3h15.4a2 2 0 0 0 1.7-3L13.7 4.4a2 2 0 0 0-3.4 0z" />
        <path d="M12 9.5v4.5M12 17.3v.01" />`,

    // 공지사항 - 확성기
    notice: `
        <path d="M3.5 9.5h3l8-4.5v14l-8-4.5h-3a1 1 0 0 1-1-1v-3a1 1 0 0 1 1-1z" />
        <path d="M6.5 14.5v4.5h3v-3.4" />
        <path d="M18.5 9.5a3.5 3.5 0 0 1 0 5" />`,

    // 사용자관리 - 사람 2명
    users: `
        <circle cx="9" cy="8" r="3.5" />
        <path d="M2.5 20.5v-1.5a4 4 0 0 1 4-4h5a4 4 0 0 1 4 4v1.5" />
        <path d="M16 4.9a3.5 3.5 0 0 1 0 6.7" />
        <path d="M17.5 15.2a4 4 0 0 1 4 4v1.3" />`,

    // 로그아웃
    logout: `
        <path d="M9.5 20.5h-4a2 2 0 0 1-2-2v-13a2 2 0 0 1 2-2h4" />
        <path d="M15.5 16.5 20 12l-4.5-4.5M20 12H9.5" />`,

    // 햄버거 메뉴
    menu: '<path d="M4 7h16M4 12h16M4 17h16" />',

    // ── 버튼·목록용 (바코드 스캔 화면) ──

    // 카메라 스캔
    camera: `
        <path d="M4 8h3l1.5-2h7L17 8h3a1 1 0 0 1 1 1v9a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V9a1 1 0 0 1 1-1z" />
        <circle cx="12" cy="13" r="3.2" />`,

    // 숫자 자판
    keypad: `
        <rect x="3.5" y="5.5" width="17" height="13" rx="1.5" />
        <path d="M7 9.5h.01M12 9.5h.01M17 9.5h.01M7 14.5h.01M12 14.5h.01M17 14.5h.01" />`,

    // 스캔 중지
    stop: '<rect x="7" y="7" width="10" height="10" rx="1.5" />',

    // 완료 (파렛트 검수·적치)
    check: `
        <circle cx="12" cy="12" r="8.5" />
        <path d="M8.5 12.2l2.4 2.4 4.6-4.8" />`,

    // 미완료
    square: '<rect x="4.5" y="4.5" width="15" height="15" rx="2.5" />',

    // 다음 차례
    /* 평치 - 바닥선 위에 놓인 파렛트 */
    floor: '<path d="M3 20h18"/><rect x="6" y="9" width="12" height="8" rx="1"/>'
        + '<path d="M9 9V6h6v3"/>',
    next: '<path d="M9 6.5l7 5.5-7 5.5z" />',

    // ── 모바일 앱 셸(m.html) ──

    // 뒤로 (상세 → 목록)
    back: '<path d="M15 5l-7 7 7 7" />',

    // 앞으로 (날짜 익일 이동) - back 의 좌우 대칭
    forward: '<path d="M9 5l7 7-7 7" />',

    // 더보기 (··· 메뉴 - 검수 초기화 · 상차완료 취소 등 위험 조작)
    more: '<path d="M6 12h.01M12 12h.01M18 12h.01" />',

    // 검색
    search: `
        <circle cx="11" cy="11" r="6.5" />
        <path d="M16 16l4.5 4.5" />`,

    // 닫기 (바텀시트)
    close: '<path d="M6 6l12 12M18 6L6 18" />',

    // 수정 - 연필
    edit: `
        <path d="M4 20h4.2l9.4-9.4a2.1 2.1 0 0 0 0-3l-1.2-1.2a2.1 2.1 0 0 0-3 0L4 15.8z" />
        <path d="M14.2 6.6l3.2 3.2" />`,

    // 저장 - 플로피 디스크
    save: `
        <path d="M4.5 4.5h11l4 4v11a1 1 0 0 1-1 1h-14a1 1 0 0 1-1-1v-14a1 1 0 0 1 1-1z" />
        <path d="M8 4.5v5h7v-5" />
        <rect x="8" y="13.5" width="8" height="6" rx="1" />`,

    // 답글 - 되돌아가는 화살표
    reply: `
        <path d="M9 6.5 4.5 11 9 15.5" />
        <path d="M4.5 11h9a6 6 0 0 1 6 6v1.5" />`,

    // 지우기 (적치 로케이션 지우기) - 휴지통
    trash: `
        <path d="M4 7h16M10 7V5h4v2" />
        <path d="M6 7l1 13h10l1-13" />`,

    // 출고적치 - 쌓아 둔 파렛트
    stow: `
        <rect x="3.5" y="4.5" width="7.5" height="7" rx="1" />
        <rect x="13" y="4.5" width="7.5" height="7" rx="1" />
        <rect x="8.25" y="13.5" width="7.5" height="7" rx="1" />`,

    // 조정요청 - 슬라이더
    adjust: `
        <path d="M3.5 8.5h9M17.5 8.5h3M3.5 15.5h3M11.5 15.5h9" />
        <circle cx="15" cy="8.5" r="2.5" />
        <circle cx="9" cy="15.5" r="2.5" />`,

    // 상차대기 - 시계
    clock: `
        <circle cx="12" cy="12" r="8.5" />
        <path d="M12 7.2V12l3.2 2" />`,

    // 재고실사표 - 문서
    sheet: `
        <path d="M6 3.5h8l5 5v12a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1v-16a1 1 0 0 1 1-1z" />
        <path d="M14 3.5V9h5" />
        <path d="M8.5 13h7M8.5 16.5h4.5" />`,

    // 업무체크리스트 - 체크 박스 목록
    checklist: `
        <rect x="3.5" y="4.5" width="6" height="6" rx="1.5" />
        <path d="M5 7.5l1.4 1.4 2.4-2.6" />
        <rect x="3.5" y="13.5" width="6" height="6" rx="1.5" />
        <path d="M12.5 7.5h8M12.5 16.5h8" />`,

    // 하위 항목 추가
    plus: '<path d="M12 5.5v13M5.5 12h13" />',

    // 순서 올리기
    up: '<path d="M6.5 14.5l5.5-5.5 5.5 5.5" />',

    // 순서 내리기
    down: '<path d="M6.5 9.5l5.5 5.5 5.5-5.5" />',

    // 메모 - 접힌 쪽지
    memo: `
        <path d="M5.5 4.5h13v10l-4 5h-9z" />
        <path d="M18.5 14.5h-4v5" />
        <path d="M8.5 8.5h7M8.5 11.5h4.5" />`,

    // 계정 - 사람 1명
    account: `
        <circle cx="12" cy="8" r="3.5" />
        <path d="M4.5 20.5v-1a5 5 0 0 1 5-5h5a5 5 0 0 1 5 5v1" />`,

    // 갈래 - 한 줄기가 두 갈래로 벌어진다 (업무프로세스의 fork)
    // 플래시(토치) - 번개
    torch: '<path d="M13 3L6.5 13.2h4.3L10 21l7.5-10.4h-4.4z" />',

    branch: `
        <circle cx="6.5" cy="12" r="2" />
        <circle cx="17.5" cy="6" r="2" />
        <circle cx="17.5" cy="18" r="2" />
        <path d="M8.5 12h3.5l3.5-5M8.5 12h3.5l3.5 5" />`,
};

/**
 * 아이콘 SVG 문자열을 만든다.
 * @param {string} name PATHS 의 키
 * @param {string} className 추가할 클래스명
 */
export function icon(name, className = 'icon') {
    const d = PATHS[name];
    if (!d) return '';
    return [
        `<svg class="${className}" viewBox="0 0 24 24" fill="none" stroke="currentColor"`,
        ' stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">',
        d,
        '</svg>',
    ].join('');
}
