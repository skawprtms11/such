...
- 탈출구는 `?shell=web` / `?shell=app`. 화면 버튼도 양쪽에 하나씩 있다 —
  앱 `계정` 화면의 `웹 화면으로`, 웹 사이드바 하단의 `앱 화면으로` (둘 다 `forceShell()`)
- ⚠️ **앱→웹 백스톱은 해시를 버리고 웹 홈으로 간다.** 웹 해시로 옮기는 표(`appRoute` 의 역방향)가
  없어서 그렇다. 앱에서 보던 화면을 웹에서 이어 보려면 웹 메뉴에서 다시 들어가야 한다
- ⚠️ **설치된 PWA 는 셸을 바꾼 당일 첫 실행이 옛 캐시로 열릴 수 있다.**
  서비스워커가 새 셸을 받아오는 것은 그 실행 이후다. 사용자에게는
  **"앱을 껐다가 다시 열어 주세요"** 로 안내한다 (재설치는 필요 없다)
