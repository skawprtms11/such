...
앱 셸(`m.html`)은 별도 상수 `config.js` 의 `APP_TAB_SETS`(하단 탭 세트 — 기본 5개 출고·검수·적치·조정·상차, 유통가공 안에서는 전용 4개) ·
`APP_MENU`(상단바 메뉴)를 쓴다.
자세한 내용은 [docs/mobile.md](mobile.md#2-하단-탭--탭-세트-configjs-의-app_tab_sets) 참고.
