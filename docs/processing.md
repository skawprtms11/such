...
- 🔑 **문서번호 바코드를 문서 맨 위 띠(`.bcband`)에 크게 넣는다** (대표님 지시 2026-09-19 —
  처음엔 머리 오른쪽 문서번호 아래 40px 였다). 바코드 아래에 문서번호 큰 글씨만 둔다 —
  안내 문구는 넣지 않는다. 앱의 검수 화면이 이 종이를 스캔해 작업을 연다(§17).
  문서번호가 없으면(아직 채번 전) 바코드 자리를 통째로 비운다
- 🔑 **띠는 1D + QR 하이브리드다** (대표님 결정 2026-09-19). **담기는 값은 둘 다 문서번호로 같다.**

  | | 코드 | 치수(실측) |
  |---|---|---|
  | 왼쪽 | `barcode.js` 의 `code128Svg(job.doc_no)` | 남는 폭 전부(약 154mm · 띠의 83% — 인쇄 폭 186mm − QR 28 − 간격 4) × 28mm · `preserveAspectRatio="none"` 이라 모듈이 같은 배율로 늘어나 규격은 유지된다 |
  | 오른쪽 | `qrcode.js` 의 `qrSvg(job.doc_no)` | **정사각 28mm** (⚠️ 비율을 깨지 않는다 — 찌그러진 QR 은 읽히지 않는다) |

  휴대폰 카메라는 QR 을, 1D 전용 하드웨어 스캐너는 Code128 을 읽는다
  ([testing.md](testing.md) 「QR 하이브리드」 · [status.md](status.md) 「상차라벨 출력」)
...
- **테스트 시트(`barcodes.html`)에 「유통가공 문서번호」 절을 더한다.** 실물 지시서를
  ...
  카드에도 Code128 옆에 같은 값의 QR 이 함께 찍힌다

구성:

```
┌──────────────────────────────────────────────┐
│ ▐█▌▐█▌▐█▌▐█▌▐█▌▐█▌▐█▌▐█▌▐█▌▐█▌▐█▌      ▒▒▒▒  │  ← 좌 Code128 · 우 QR (같은 문서번호)
│                20260919-01                   │
├──────────────────────────────────────────────┤
