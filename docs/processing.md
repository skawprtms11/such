...
- 🔑 **문서번호 Code128 바코드를 문서 맨 위 띠(`.bcband`)에 크게 넣는다** (대표님 지시 2026-09-19 —
  처음엔 머리 오른쪽 문서번호 아래 40px 였다). 인쇄 폭을 꽉 채우고(세로 28mm · `preserveAspectRatio=none`)
  바코드 아래에 문서번호 큰 글씨만 둔다 — 안내 문구는 넣지 않는다.
  앱의 검수 화면이
