...
- 🔑 **이미 등록된 주문은 상세 팝업의 `주문합치기` 버튼으로 나중에 묶을 수 있다.**
  같은 거래처·같은 등록자·같은 출고요청일의 주문을 골라 `db.mergeOrders()` 로
  대표주문번호를 맞춘다 (대상에 대표주문번호가 없으면 대상의 주문번호가 대표가 된다).
  ⚠️ **검수작업 전에만 합칠 수 있고, 차수 형제(`base_no` 같음)는 합칠 수 없다.**
  검수완료 시 총 파렛트수·박스수를 묶음 대표에만 입력하기 때문이고, 차수를
  대표주문번호로 또 묶으면 출고·검수까지 묶여 나중 차수가 막히기 때문이다
  ([docs/orders.md](docs/orders.md) 의 허용 조건 ①~⑧ 표 참고)
...
- 🔑 **주문합치기는 상차검수 전까지 할 수 있다** (검수작업이 끝난 뒤도 된다).
  대신 **한 상차 묶음이 될 주문 전부의 진행 상태가 같아야 한다**
  (`inspect_done_at` · `stow_done_at` · `extra_done_at` 유무 + `readyToLoad`).
  비교 모집단은 `groupOf` — 상대에게 딸린 추가주문 차수까지 본다
  ([docs/orders.md](docs/orders.md) ④ ④-2)
- 🔑 **`db.completeLoading()` 은 `loaded_at` 을 찍기 전에 멤버마다 `readyToLoad` 를
  다시 확인한다.** 상차검수 상태만 보면 검수·적치를 건너뛴 주문이 묶음에 딸려
  상차완료·마감된다 ([docs/loading.md](docs/loading.md))
...
- 🔑 **묶음에는 두 종류가 있고 파렛트가 있는 곳이 다르다. 둘 다 정상이다.**

  | 묶음 종류 | 파렛트가 있는 곳 | 상차라벨 | 상차검수 |
  |---|---|---|---|
  | 등록 시 대표주문번호로 묶음 | 검수 시 총량을 **대표에 1회** 입력 → 멤버는 0파렛트 | 대표번호 라벨 1종 | 대표 바코드를 총량만큼 |
  | **검수 후 합침** ([docs/orders.md](docs/orders.md)) | **멤버마다 자기 파렛트** | **주문마다 자기 라벨** | 주문별 라벨을 각각 그 수량만큼 |

  **묶음 총량 = 멤버 `pallet_count` 합 · 멤버 `box_count` 합**으로 읽으면 두 종류 모두
  맞는다. 화면은 저장값(대표의 `pallet_count`)이 아니라 이 합계를 쓴다 (목록·상세·라벨 전부).
  두 종류를 가르는 **판정은 `db.hasSplitPallets(rows)` 한 곳**뿐이고
  (기준 = 자기 총량을 가진 멤버가 2건 이상인가), 모집단은 부르는 화면이 정한다 —
  라벨·총량 수정은 **대표주문번호 묶음**, 상차검수 안내는 **상차 묶음**.
  총량 수정 대상은 `db.countTarget(orderId)` 이 알려주고,
  `db.setPalletCount` · `db.setBoxCount` 는 **넘긴 주문을 그대로** 고친다
...
- **상차라벨 출력 경로는 묶음 종류에 따라 갈린다.** 등록 시 묶은 묶음은 대표주문번호로
  찍고(바코드·큰 글씨 = `rep_no`, 묶인 주문번호는 작은 행, 파렛트수는 묶음 총량)
  출력 버튼이 **묶음 대표 행에만** 있다. **검수 후 합친 묶음은 이미 주문별 라벨이
  출력돼 있어** 주문마다 자기 번호·자기 파렛트수로 출력한다.
...
| `process_master_files` | 유통가공 **작업가이드 파일 메타** (파일은 Storage 버킷 `process-guides`) | `master_id`(on delete cascade) `name`(원본 파일명) `path`(`guides/{master_id}/{seq}.{jpg\|pdf}`) `mime`(image/jpeg · application/pdf) `size` `sort_order`(인쇄 순서 = 경로의 seq) `created_by(_name)` · 작업지시서 인쇄 시 **이미지가 뒤 페이지로 붙는다** |
